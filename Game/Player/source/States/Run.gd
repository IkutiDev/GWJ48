...
func physics_process(delta: float) -> void:
	...
	if owner.is_on_floor():
		if move.DECCELERATION_ENABLED and move.velocity.length() < 1.0 and move.get_move_direction(owner.player_combat.stop_movement).x == 0:
			_state_machine.transition_to("Move/Idle")
		elif not move.DECCELERATION_ENABLED and move.get_move_direction(owner.player_combat.stop_movement).x == 0:
			_state_machine.transition_to("Move/Idle")
	else:
